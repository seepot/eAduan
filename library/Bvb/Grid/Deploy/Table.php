...
class Bvb_Grid_Deploy_Table extends Bvb_Grid_DataGrid
{
	...
	/***************** Column *********************/
	public $_arrColumn = array();
	public $_arrCurColumn = array();
	
	/***************** Legend *********************/
	public $_arrLegend = array();
	
	/***************** Controller untuk Ikon Aktiviti *********************/
	public $_iconController = '';
	/***************** Nama ID *********************/
	public $_id = 'id';
	/***************** Customize untuk Premis *********************/
	public $_tpl = array();
	public $type_id = '';
    /**
    ...
    function buildTitltesTable($titles)
    {
		...
		//select column
		$column = "<a href=\"#\" onClick=\"document.getElementById('sel_column').style.display=document.getElementById('sel_column').style.display=='block' ? 'none' : 'block'; return false;\"><img title=\"Select Column\" src=\"".IMAGES_PATH."icon/selectcols.png\"></a>";
		
        //[PT]Finalizaro template nos títulos
         $grid .= str_replace ( '{{column}}', $column, $this->temp ['table']->titlesEnd () );
    ...
    function buildGridTable($grids)
    {
		...
		*/
        foreach ( $grids as $key => $value )
        {
            ...
            {
				//print_r($final);
				if($this->_arrLegend <> array())
				{
					//echo $final['field'].' | '.$this->_arrLegend['field'].'<br>';
					if($final['field'] == $this->_arrLegend['field'] && $final['value'] <> NULL) //detect field untuk legend
					{
						foreach ( $this->_arrLegend['item'] as $item)
						{
							//print_r($item);
							$diff = strtotime("now") - strtotime($final['value'])."<br>";
							if($item['max'] <> ''){
								if($diff > (int)$item['min'] && $diff < (int)$item['max'])
									$class = $item['class'];
							}else {
								if($diff > (int)$item['min'])
									$class = $item['class'];
							}
						}
					}
			...
			//echo $class;
            $grid .= $this->temp ['table']->loopStart ( $finalFields, $ppagina, $actual, $class );
            $set = 0;
			...
			if (count($this->_arrIcon) > 0){
				foreach($this->_arrIcon as $i)
				{	
					if(isset($i['onclick']['item'])) {
						/* echo "<pre>";
						print_r($i['onclick']);
						echo "</pre>"; */
						foreach ( $value as $final )
						{
							for($n=0;$n<count($i['onclick']['item']);$n++)
							{
								if ($i['onclick']['item'][$n] == $final['field']){
									$item[$n] = $final['value'];
								}
							}
							
						}
						$newIcon = $icon->func_createIcon3($i['icon'],$i['target'],$i['title'],$i['url'],$icon->func_createOnClick2($i['onclick']['name'],$i['onclick']['type'],$item));
					}
					else {
						$newIcon = $this->_arrIcon;
					}
			...
			foreach ( $value as $final )
            {
				if($final['field'] == 't.category_type_id'){
					$this->type_id = $final['value'];
				}
			}
            foreach ( $value as $final )
            {
				...
				//cari controller ikon aktiviti////
				//echo $final['field'];
				if($final['field'] == $this->_iconController)
				{
					$_ctrlValue = $final['value'];
				}
                ...
                if (($finalField != $finalHrow && isset ( $this->info ['hRow'] ['title'] )) || ! isset ( $this->info ['hRow'] ['title'] ))
                {
                    ...
                    $set ++;
						...
						if(!in_array($final['field'],$this->_tpl)){
							$grid .= str_replace ( array ("{{value}}", "{{class}}", "{{class2}}" ), array (preg_replace("/\n/","<br/>",$final ['value']), $final ['class'], $class ), $this->temp ['table']->loopLoop ( $finalFields ) );
						}else{
							$str_final = '';
							if($final['field'] == 'r.premise_classification'){
								
								//echo $this->type_id;
								for($i=0;$i<$final['value'];$i++){
									if($this->type_id == 1 || $this->type_id == 2){
										$str_final .= '<img src="'.IMAGES_PATH.'/icon/goldstar.gif">';
									} else if($this->type_id == 3) {
										$str_final .= '<img src="'.IMAGES_PATH.'/icon/orkid.png">';
									}//echo $str_final;
								}
								$grid .= str_replace ( array ("{{value}}", "{{class}}", "{{class2}}" ), array ($str_final, $final ['class'], $class ), $this->temp ['table']->loopLoop ( $finalFields ) );
								//echo $final['value'];
							} else if($final['field'] == 'r.premise_green'){
								if($final['value'] == 1)
									$str_final = '<img src="'.IMAGES_PATH.'/icon/active.gif">';
								$grid .= str_replace ( array ("{{value}}", "{{class}}", "{{class2}}" ), array ($str_final, $final ['class'], $class ), $this->temp ['table']->loopLoop ( $finalFields ) );
							}
						}	
					}
			...
			//$_icon = $this->_arrIcon;
			$_icon = $newIcon;
            $grid .= $this->temp ['table']->loopEnd ( $_view, $_edit, $_delete, $_other, $_icon, $id[$key], $this->_iconController, $_ctrlValue, $this->_id );
    ...
    function pagination()
    {
        $url = parent::getUrl ( array ('start', 'paging','submit_col','sel_column' ) );
        ...
        } else
        {
            ...
            $pag = ($actual == 1) ? '<strong>1</strong>' : "<a href=\"$url/start/0\">1</a>";
            //$pag = ($actual == 1) ? '<img src="'.IMAGES_PATH.'admin/pager_arrow_left_off.gif">' : "<a href=\"$url/start/0\"><img src=\"".IMAGES_PATH."admin/pager_arrow_left.gif\"></a>";
		...
		$pag = "Page ";
		$pag .= ($actual != 1) ? "<a href=\"$url/start/" . (($actual - 2) * $ppagina) . "\"><img src=\"".IMAGES_PATH."admin/pager_arrow_left.gif\"></a>" : "<a href=\"#\"><img src=\"".IMAGES_PATH."admin/pager_arrow_left_off.gif\"></a>";
		//$pag .= "<input type=\"text\" name=\"page\" value=\"$actual\" style=\"width:2em !important;\" class=\"input-text page\" onchange=\"Javascript:func_page(".$this->pagination.",this.value,".$url.")\"/>";
		//$pag .= "<input type=\"text\" name=\"page\" value=\"$actual\" style=\"width:2em !important;\" class=\"input-text page\" onchange=\"window.location='{$url}/start/'+((this.value*".$this->pagination.")-".$this->pagination.")\"/>";
		//$pag .= "<input type=\"text\" name=\"page\" id=\"page\" value=\"$actual\" style=\"width:2em !important;\" class=\"input-text page\" onchange=\"alert(this.value)\"/>";
		$pag .= "<input type=\"text\" name=\"page\" id=\"page\" value=\"$actual\" style=\"width:2em !important;\" class=\"input-text page\" onchange=\"this.value > ".$npaginas." ? '' : window.location='{$url}/start/'+((this.value*".$this->pagination.")-".$this->pagination.");\"/>";
		//$pag .= "<input type=\"text\" name=\"page\" id=\"page\" value=\"$actual\" style=\"width:2em !important;\" class=\"input-text page\" onChange=\"this.value > ".$npaginas." ? 'alert(1)' : ''; return false;\"/>";
		$pag .= ($actual != $npaginas) ? "<a href=\"$url/start/" . ($actual * $ppagina) . "\"><img src=\"".IMAGES_PATH."admin/pager_arrow_right.gif\"></a>" : "<a href=\"#\"><img src=\"".IMAGES_PATH."admin/pager_arrow_right_off.gif\"></a>";
		$pag .= "&nbsp;of";
		$pag .= "&nbsp;".$npaginas;
		$pag .= "&nbsp;pages&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;";
        ...
        /* if ($actual != 1)
        {
            ...
            } else
            {
               ...
               // $pag = " <a href=\"$url/start/0\">" . $this->__ ( 'First' ) . "</a>&nbsp;&nbsp;<a href=\"$url/start/" . (($actual - 2) * $ppagina) . "\"><img src=\"".IMAGES_PATH."admin/pager_arrow_left_off.gif\"></a>&nbsp;&nbsp;" . $pag;
            }
		...
		//echo $this->pagination." ".Zend_Controller_Front::getInstance()->getRequest()->getParam('paging');
		//number per page
		$arr_pag = array('5' => 5, '10' => 10, '15' => 15, '20' => 20, '30' => 30, '50' => 50, '100' => 100);
		$paging = "<select id=\"paging\" onchange=\"window.location='{$url}/paging/'+this.value\">";
		foreach($arr_pag as $key=>$value){
			$paging .= "<option ";
			if (Zend_Controller_Front::getInstance()->getRequest()->getParam('paging') == $value || $this->pagination == $value)
			{
				$paging .= " selected ";
			}
		...
		//select column
		$column = "<a title=\"Select Column\" href=\"#\" onClick=\"document.getElementById('sel_column').style.display=document.getElementById('sel_column').style.display=='block' ? 'none' : 'block'; return false;\"><img src=\"".IMAGES_PATH."icon/selectcols.png\"></a>";
		//$column .= "<div id=\"sel_column\" style=\"display:none\">";
        ...
        if ($npaginas > 1 || count ( $this->export ) > 0)
        {
            ...
            {
                $registoActual = $actual * $ppagina - $ppagina + 1;
            ...
            if (( int ) @$this->info ['limit'] > 0)
            {
				...
				$result2 = str_replace ( array ('{{export}}', '{{column}}', '{{pagination}}', '{{pageSelect}}', '{{pagingNo}}', '{{numberRecords}}' ), array ($exp, $column, '', '', $paging, ( int ) $this->info ['limit'] ), $this->temp ['table']->pagination () );
            ...
            } elseif ($npaginas > 1 && count ( $this->export ) > 0)
            {
                ...
                //echo $f;
                $result2 = str_replace ( array ('{{export}}', '{{column}}', '{{pagination}}', '{{pageSelect}}', '{{pagingNo}}','{{numberRecords}}' ), array ($exp, $column, $pag, $f, $paging, $registoActual . ' ' . $this->__ ( 'to' ) . ' ' . $registoFinal . ' ' . $this->__ ( 'of' ) . '  ' . $this->_totalRecords ), $this->temp ['table']->pagination () );
			...
			/* 	
				...
				$result2 = str_replace ( array ('{{export}}', '{{column}}', '{{pagination}}', '{{pageSelect}}', '{{pagingNo}}', '{{numberRecords}}' ), array ($exp, $column, $pag, $f, $paging, $registoActual . ' ' . $this->translate()->_( 'hingga' ) . ' ' . $registoFinal . ' ' . $this->translate()->_( 'dari' ) . '  ' . $this->_totalRecords ), $this->temp ['table']->pagination () );
            ...
            } elseif (count ( $this->export ) == 0)
            {
                ...
                $result2 .= str_replace ( array ('{{export}}', '{{column}}', '{{pagination}}', '{{pageSelect}}', '{{pagingNo}}', '{{numberRecords}}' ), array ('', $column, $pag, $f, $paging, $this->_totalRecords ), $this->temp ['table']->pagination () );
            
            }
			 */
			
			//$result2 = str_replace ( array ('{{export}}', '{{column}}', '{{pagination}}', '{{pagingNo}}','{{numberRecords}}' ), array ($exp, $column, $pag, $paging, $registoActual . ' ' . $this->__ ( 'to' ) . ' ' . $registoFinal . ' ' . $this->__ ( 'of' ) . '  ' . $this->_totalRecords ), $this->temp ['table']->pagination () );
			$result2 = str_replace ( array ('{{export}}', '{{column}}', '{{pagination}}', '{{pagingNo}}','{{numberRecords}}' ), array ($exp, $column, $pag, $paging, $this->_totalRecords ), $this->temp ['table']->pagination () );
	...
	//function build End untuk wat select column
	function buildEnd()
    {
		$url = parent::getUrl ( array('submit_col','sel_column') );
		$column2 = "<form method=\"post\" action=\"".$url."\">";
		$column2 .= "<fieldset id=\"sel_column\" style=\"z-index: 2; position: absolute; right: 10px; top: 83px; background-color: white; width: 100px; padding: 10px; border: #cccccc 2px solid; display:none;width:200px;text-align:right;display:none;width:200px;text-align:right;\"><legend><strong>Select columns</strong></legend>";
		$column2 .= "<table><tr><td align=\"left\">";
		$column2 .= "<div onFocus=\"self.status='Select the columns to display in the list'; return true;\" onBlur=\"self.status=''; return true;\" style=\"height: 8em; width: 190px; background-color: white; overflow: auto; border: lightgray 2px inset; text-align: left;\">";
		foreach($this->_arrColumn as $col=>$name){
			$column2 .= "<input type=\"checkbox\" name=\"sel_column[]\" id=\"sel_column[]\" value=\"$col\" ";
			if (array_key_exists($col, $this->_arrCurColumn)) {
				$column2 .= "checked=\"1\"";
			}
			$column2 .= ">";
			$column2 .= "&nbsp;<label>$name</label>";
			$column2 .= "<br>";
		} 
		
		/* foreach($this->_arrColumn as $col){
			$column2 .= "<input type=\"checkbox\" name=\"sel_column[]\" id=\"sel_column[]\" value=\"".$col['id']."\" ";
			foreach($this->_arrCurColumn as $cur){
				if ($col['id'] == $cur['id']) {
					$column2 .= "checked=\"1\"";
				}
			}
			$column2 .= ">";
			$column2 .= "&nbsp;<label>".$col['name']."</label>";
			$column2 .= "<br>";
		} */
		$column2 .= "</div>";
		$column2 .= "</td></tr>";
		$column2 .= "<tr><td><button type=\"submit\" name=\"submit_col\" id=\"submit_col\"  />Hantar</button>&nbsp;";
		$column2 .= "<button type=\"button\" onclick=\"document.getElementById('sel_column').style.display='none';\">Tutup</button></td>";
		$column2 .= "</table>";
		$column2 .= "</fieldset>";
		$column2 .= "</form>";
		
		$result3 = str_replace ( array ('{{column2}}' ), array ($column2), $this->temp ['table']->globalEnd () );
	...
	/**********************List  untuk Select Column************************/
	function listColumn($all_column = array(), $cur_column = array())
	{
		$this->_arrColumn = $all_column;
		$this->_arrCurColumn = $cur_column;
	}
	
	/*********************Legend****************************/
	function setLegend($legend = array())
	{
		$this->_arrLegend = $legend;
	}
	
	/*********************Setting Kawalan Ikon Untuk Aktiviti****************************/
	function setIconController($field = '')
	{
		$this->_iconController = $field;
	}
	
	/*********************Setting Nama ID Aktiviti****************************/
