...
class Bp_PenyataController extends Spt_Controller_Action
{
	...
	public function carianAction()
	{
		...
		// Connect to MSSQL
		$link = mssql_connect($server, 'pub', 'pub');
	...
	public function tahun2012Action()
	{
		...
		$this->view->tahun = $tahun = $this->_getParam('x');
		...
		$table = new AudiTrail;
			$data = array(
				...
				'audit_trail_module' => Zend_Filter::filterStatic('Semak Penyata', 'HtmlEntities'),
				'audit_trail_task' => Zend_Filter::filterStatic('Penyata '.$tahun.' - '.Zend_Auth::getInstance()->getIdentity()->usr_id, 'HtmlEntities'),
				'audit_trail_datetime' => date('Y-m-d H:i:s'),
				'audit_trail_ipaddress' => $_SERVER['REMOTE_ADDR']
			);
			$table->insert($data);
	...
	public function tahun2010xAction()
	{
		...
		$table = new AudiTrail;
			$data = array(
				'usr_id' => Zend_Filter::get(Zend_Auth::getInstance()->getIdentity()->usr_id, 'HtmlEntities'),
				'audit_trail_module' => Zend_Filter::get('Semak Penyata', 'HtmlEntities'),
				'audit_trail_task' => Zend_Filter::get('Penyata - '.Zend_Auth::getInstance()->getIdentity()->usr_id, 'HtmlEntities'),
				'audit_trail_datetime' => date('Y-m-d H:i:s'),
				'audit_trail_ipaddress' => $_SERVER['REMOTE_ADDR']
			);
			$table->insert($data);
