...
class My_Template_Table_Test extends Bvb_Grid_Template_Table_Table
{
    ...
    function loopEnd ($view, $edit, $delete, $other, $arr_icon, $id, $field, $values,$idname)
    {
		...
		foreach($arr_icon as $name=>$value){
			...
			$x = explode('/',str_replace(SYSTEM_URL,'/',$value['url']));
			//echo SYSTEM_URL;
			//print_r($x);
			//die();
			...
			if ($role == 2 || $acl->isAllowed($role, $x[2].'_'.$x[3], $x[4])){
