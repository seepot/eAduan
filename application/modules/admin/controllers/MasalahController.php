...
class Admin_MasalahController extends Spt_Controller_Action
{
	public function indexAction()
	{
		$this->_redirect('/admin/masalah/list');
	}
	...
	public function listAction()
	{
		...
		//column
		$arr_col = array('m.km_id' => 'Id','m.km_desc' => 'Kategori Masalah','n.bpm_name' => 'Unit BPM');
		...
		elseif(isset($defSession->col))
			...
			$arr_col_1 = array('m.km_id' => 'Id','m.km_desc' => 'Kategori Masalah','n.bpm_name' => 'Unit BPM');
		
		$xtvt_id = 'm.km_id'; //aktiviti key
		$this->title = $this->translate()->_("Senarai Kategori Masalah");
		$grid = $this->grid ( 'table' );
		$grid->from('ea_kategoriMasalah m INNER JOIN ea_bpm n ON m.bpm_id = n.bpm_id');
		$grid->table( array ( 'm' => 'ea_kategoriMasalah', 'n' => 'ea_bpm' ) );
		...
		$grid->activityId ( $xtvt_id );
		//////
		$grid->editButton ( true, $this->translate()->_("Kemaskini Unit"), Zend_Controller_Front::getInstance()->getBaseUrl().'/admin/masalah/edit' );
		$grid->deleteButton ( true, $this->translate()->_("Hapus Unit"), Zend_Controller_Front::getInstance()->getBaseUrl().'/admin/masalah/delete' );
	...
	public function addAction()
	{
		...
		$form = new Spt_Form_Admin_Masalah_Add;
		...
		$data = array(
            'km_desc' => Zend_Filter::filterStatic($values['txt_masalah_name'], 'HtmlEntities'),
            'bpm_id' => Zend_Filter::filterStatic($values['sel_bpm'], 'HtmlEntities')
        );
		$db->insert('ea_kategorimasalah', $data);
    ...
    {
    	$form = new Spt_Form_Admin_Masalah_Edit;
    	...
    	if (!$this->getRequest()->isPost()) {
            $rowset = $db->fetchRow(
					$db->select()
							->from(array('m' => 'ea_kategorimasalah'),
								array('km_desc','bpm_id'))
							->where('km_id = ?', $this->_getParam('id'))
        ...
        $where[] = "km_id = '".$this->_getParam('id')."'";
		$db->update('ea_kategorimasalah', $data, $where );
	...
	public function deleteAction()
    {
    	...
    	$where = 'km_id = "' . $id . '"';
		$db->delete ( 'ea_kategorimasalah', $where );
		$this->view->entrySaved = true;
		$this->_redirect('/admin/masalah/list');
    }
	...
	protected function _repopulateForm($form, $item)
    {
    	...
    	$values = array(
        	'txt_masalah_name' => $this->_reverseAutoFormat($item['km_desc']),
        	'sel_bpm' => $this->_reverseAutoFormat($item['bpm_id'])
        );
