...
class My_Template_Table_Work extends Bvb_Grid_Template_Table_Table
{
    ...
    function loopEnd ($view, $edit, $delete, $other, $arr_icon, $id)
    {
        //print_r($arr_icon);
		...
		foreach($arr_icon as $name=>$value){
			$t_icon = $name.'.png'; //jenis ikon
			$target = $value['target']; //target
			$title = $value['title']; //title
			$url = $value['url']."/id/".$id; //url
	...
	function pagination()
    {
		...
		return "<table width=\"100%\"><tr class=\"barra_tabela\"><td class=\"barra_tabela\" colspan=\"".($this->colSpan+2)."\"><div style=\"padding:2px;\">
        <div style=\"float:left;text-align:left;width:600px;\"> {{pagination}} View {{pagingNo}} per page&nbsp;|&nbsp;Total {{numberRecords}} records&nbsp;|&nbsp;
		<button type=\"button\" class=\"search2\" onClick=\"filters()\"><span>". $this->translate()->_('Carian'). "</span></button>
		</div>
        <div style=\"text-align:right;float:right;width:200px;\">{{export}} | {{column}}</div>
		</div>
        </td></tr></table>";
