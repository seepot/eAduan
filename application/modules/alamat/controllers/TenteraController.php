...
class Alamat_TenteraController extends Spt_Controller_Action
{
	...
	public function indexAction()
	{
		...
		$this->_redirect('/alamat/tentera/list');
	}
	...
	public function listAction()
	{
		...
		//column
		$arr_col = array('t.notentera' => 'No Tentera','t.nama' => 'Nama Tentera','t.alamat' => 'Alamat');
		...
		elseif(isset($defSession->col))
			...
			$arr_col_1 = array('t.notentera' => 'No Tentera','t.nama' => 'Nama Tentera','t.alamat' => 'Alamat');
		
		$xtvt_id = 't.notentera'; //aktiviti key
		$this->title = $this->translate()->_("Senarai Tentera");
		$this->view->url = $this->getUrl();
		$grid = $this->grid ( 'table' );
		$grid->from('tbl_tentera t');
		//$grid->where('u.type_id=1');
		$grid->table( array ( 't' => 'tbl_tentera') );
		$grid->activityId ( 't.notentera' );
		...
		$grid->order('t.notentera ASC')
			->setPagination ( $pagingNo )
			->listColumn($arr_col, $arr_col_1);
		
		$grid->setTemplate ( 'work', 'table' );
		$grid->viewButton ( true, $this->translate()->_("Papar"), Zend_Controller_Front::getInstance()->getBaseUrl().'/alamat/tentera/view' );
